...
const connect = (firebase, uuid) => {
    ...
    const updateTrip = status => new Promise((resolve, reject) => {
        const ref = firebase.database().ref('event-queue/driver/trip/request');
    const refResult = firebase.database().ref('event-queue/driver/trip/result');
    ...
    if (status === 'start') {
        payload.status.startTrip = true;
    } else {
        payload.status.endTrip = true;
    }

    ref.child(newKey).set(payload).then(() => {
        refResult.child(newKey).on('value', (snapshot) => {
        const { onProgress, success, error } = snapshot.val() || {};
    if (onProgress != null && onProgress === false) {
        console.log(`${status} trip for ${uuid} requested....`);
        refResult.child(newKey).off();
        refResult.child(newKey).remove();

        if (success === true && status === 'start') {
            setTimeout(() => updateTrip('end'), 3000);
        }
        return resolve();
    }
    return null;
